  ...
  return {
    ...
    ur: parsed.ur
      ...
      if (rainChance > 60 || tomorrowRain > 60) {
        wateringAdviceEn = `🌧️ Heavy rain expected - Skip watering for 2 days\n💧 Natural irrigation will suffice; monitor soil drainage`;
        wateringAdviceUr = `🌧️ شدید بارش کی توقع - دو دن تک پانی نہ دیں\n💧 قدرتی آبپاشی کافی ہوگی؛ مٹی کی نکاسی کی نگرانی کریں`;
      } else if (rainChance > 40) {
        wateringAdviceEn = `🌦️ Rain likely - Skip watering today\n☔ Check fields after rain for waterlogging`;
        wateringAdviceUr = `🌦️ بارش کا امکان - آج پانی نہ دیں\n☔ بارش کے بعد کھیتوں میں پانی جمع ہونے کی جانچ کریں`;
      } else if (temp > 35) {
        wateringAdviceEn = `🌡️ Hot weather (${temp}°C, feels like ${feelsLike}°C) - Water early morning (${sunrise}-8 AM) or evening (6 PM-${sunset})\n💧 Deep root watering to minimize evaporation and combat heat stress`;
        wateringAdviceUr = `🌡️ گرم موسم (${temp}°C, محسوس ہونے والی ${feelsLike}°C) - صبح سویرے (${sunrise}-8 AM) یا شام (6 PM-${sunset}) کو پانی دیں\n💧 بخارات کم کرنے اور گرمی کے دباؤ سے لڑنے کے لیے جڑوں پر گہرا پانی دیں`;
      } else if (temp > 30) {
        ...
        if (Math.abs(dewPoint - temp) < 3) {
          wateringAdviceEn += `\n⚠️ High dew point - Avoid evening watering to prevent fungal growth`;
          wateringAdviceUr += `\n⚠️ زیادہ ڈیو پوائنٹ - فنگل کی نشوونما روکنے کے لیے شام کو پانی نہ دیں`;
        }
      } else {
      ...
      if (windSpeed > 7) {
        ...
        sprayingAdviceEn = `⚠️ Windy conditions (${windSpeed} m/s) - Avoid spraying, wait for calm weather\n🧭 Direction ${windDir} may cause uneven coverage`;
        sprayingAdviceUr = `⚠️ تیز ہوا (${windSpeed} m/s) - سپرے نہ کریں، پرسکون موسم کا انتظار کریں\n🧭 سمت ${windDir} غیر یکساں کوریج کا سبب بن سکتی ہے`;
      } else if (rainChance > 40) {
        sprayingAdviceEn = `🌧️ Rain expected - Wait, spray will wash away\n⏰ Reschedule after rain clears`;
        sprayingAdviceUr = `🌧️ بارش متوقع - انتظار کریں، سپرے بہہ جائے گا\n⏰ بارش صاف ہونے کے بعد دوبارہ شیڈول کریں`;
      } else if (temp > 35 || uvIndex > 8) {
        sprayingAdviceEn = `🌡️ Too hot/High UV (${uvIndex}) - Spray early morning (6-8 AM) or evening (after ${sunset})\n☀️ Avoid midday to prevent leaf burn and chemical volatility`;
        sprayingAdviceUr = `🌡️ بہت گرم/زیادہ UV (${uvIndex}) - صبح سویرے (6-8 AM) یا شام ( ${sunset} کے بعد) سپرے کریں\n☀️ پتوں کے جلنے اور کیمیکل کی اتار چڑھاؤ سے بچنے کے لیے دوپہر سے گریز کریں`;
      } else if (currentHour >= 10 && currentHour <= 16) {
        sprayingAdviceEn = `☀️ Best time now - Good conditions for spraying\n✅ Low wind (${windSpeed} m/s), moderate temp for even application`;
        sprayingAdviceUr = `☀️ اب بہترین وقت - سپرے کے لیے اچھے حالات\n✅ کم ہوا (${windSpeed} m/s)، معتدل درجہ حرارت یکساں استعمال کے لیے`;
      } else {
      ...
      if (temp > 40 || feelsLike > 45) {
        ...
        protectionEn = `🌧️ Heavy rain coming - Protect delicate crops, ensure drainage\n🏞️ Clear ditches; elevate seedlings`;
        protectionUr = `🌧️ شدید بارش آ رہی ہے - نازک فصلوں کو محفوظ رکھیں، پانی کی نکاسی کا بندوبست کریں\n🏞️ نالیاں صاف کریں؛ انکلنگ اٹھائیں`;
      } else if (uvIndex > 8) {
        protectionEn = `☀️ Extreme UV (${uvIndex}) - Use full shade nets 10 AM-4 PM\n🛡️ Prevents sunburn on leaves and fruits`;
        protectionUr = `☀️ انتہائی UV (${uvIndex}) - 10 AM-4 PM مکمل شیڈ نیٹس استعمال کریں\n🛡️ پتوں اور پھلوں پر دھوپ کے جھلسنے سے بچائیں`;
      } else if (uvIndex > 5) {
        protectionEn = `☀️ High UV (${uvIndex}) - Light shade for sensitive crops\n🌱 Especially seedlings and leafy greens`;
        protectionUr = `☀️ زیادہ UV (${uvIndex}) - حساس فصلوں کے لیے ہلکا سایہ\n🌱 خاص طور پر انکلنگ اور پتوں والی سبزیاں`;
  ...
  useEffect(() => {
    const fetchWeatherAndGetAdvice = async () => {
      ...
      try {
        ...
        console.log('Fetching from URLs:', { weatherUrl, uvUrl });
          ...
          en: {
            ...
            sprayingAdvice: `🌾 General Spraying Guidelines:\n` +
              `${isEarlyMorning ? "✅ Early morning is ideal for spraying - low wind, good absorption.\n" : 
                isMidDay ? "⚠️ Avoid spraying during hot midday hours.\n" : 
            ...
            protection: `🛡️ General Protection Measures:\n` +
              `${isMidDay ? "⚠️ Protect plants from intense midday sun.\n" : ""}` +
          ...
          ur: {
            ...
            sprayingAdvice: `🌾 عام سپرے کے اصول:\n` +
              `${isEarlyMorning ? "✅ صبح سویرے سپرے کرنا مثالی ہے - کم ہوا، بہتر جذب۔\n" : 
                isMidDay ? "⚠️ دوپہر کی تیز دھوپ میں سپرے سے بچیں۔\n" : 
            ...
            protection: `🛡️ عام حفاظتی اقدامات:\n` +
              `${isMidDay ? "⚠️ دوپہر کی تیز دھوپ سے پودوں کو بچائیں۔\n" : ""}` +
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-2">
                  <span className="text-2xl">🌡️</span>
                  <div>
                    <div className="flex justify-betweenitems-center gap-x-4">
                      <p className="text-sm text-gray-600">Temperature</p>
                      <p className="text-sm text-gray-600 urdu-font mr-2" style={{ fontFamily: 'Noto Nastaliq Urdu, serif' }}>درجہ حرارت</p>
                    </div>
